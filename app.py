...
from scripts import knn,random_forest,NB_and_RF,neural_network,naive_bayes
...
def iris():
    return render_template('iris.html',active=['','active',''],knn=knn_iris,rf=Rf_iris,nb=bayes_iris,nn=neuronal_iris,lr=logic_iris)

@app.route('/wine',methods = ['Get'])
def wine():
    return render_template('wine.html',active=['','','active'],knn=knn_wine,rf=Rf_NB_wine[1],nb=Rf_NB_wine[0],nn=neuronal_wine,lr=logic_wine)
